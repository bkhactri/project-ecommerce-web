...
exports.getIndex = async(req, res, next) => {
    ...
    const menProducts = await Product.find({
            filter: { $regex: "men", $options: "i" },
        })
        .limit(9)
        .exec();
    const womenProducts = await Product.find({
            filter: { $regex: "woman", $options: "i" },
        })
        .limit(8)
        .exec();
...
exports.getProduct = async(req, res, next) => {
    ...
    const relatedProducts = await Product.find({
            $and: [
                { filter: { $regex: product.filter, $options: "i" } },
                { _id: { $ne: product._id } },
            ],
        })
        .limit(10)
...
exports.postOrder = async(req, res, next) => {
    ...
    const create_moment = `${today.getHours()}:${today.getMinutes()}:${today.getSeconds()} - T${
    today.getDay() + +1
    ...
    req.user
        ...
        .then((result) => {
            products.forEach(async(p) => {
                const update = await Product.findById(p.product._id).then((product) => {
                    if (!product.hasSold) {
                        product.hasSold = 0;
                    }
                    product.hasSold++;
