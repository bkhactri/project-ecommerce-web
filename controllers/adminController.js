...
const {
    ...
    toArrayColor,
    removeAccents
} = require('../models/service/module.js');
...
exports.getInvoice = async (req, res, next) => {
    ...
    pdfDoc
        .fontSize(20)
        .text('Coza Store', 45, 100, {
            align: "left"
        });
    pdfDoc
        .fontSize(13)
        .text('379 Hudson St,New York, NY 10018', 45, 130, {
            align: "left"
        })
        .text('Phone: (+1) 96 716 6879', 45, 150, {
            align: "left"
        })
        .text(`Invoice #${order._id}`, 200, 100, {
            align: "right"
        })
        .text(`Order date: ${order.dateShow}`, 200, 120, {
            align: "right"
        })
    ...
    order.products.forEach(p => {
        pdfDoc
            ...
            .text(eval(p.quantity * p.product.price), 520, vertical, {
                align: "right"
            })
        vertical += 20;
    });
    pdfDoc.text('------------------------------------', 430, vertical, {
        align: "right"
    });
    //total end invoice
    vertical += 20;
    pdfDoc
        ...
        .text(order.totalPrice.toFixed(2), 520, vertical, {
            align: "right"
        });
    vertical += 20;
    pdfDoc
...
exports.postCustomers = async (req, res, next) => {
    ...
    }).skip((page - 1) * USERS_PER_PAGE).limit(USERS_PER_PAGE);
    res.render('admin/customer-list', {
        pageTitle: 'Customer list',
        path: '/admin/customers',
        user: req.admin,
...
exports.postAddAdmin = async (req, res, next) => {
    ...
    let isValidEmail = await Admin.find({
        email: email
    });
