    ...
    passport.authenticate("local.signin", function (err, user, info) {
        if (info) {
            ...
            if (message == "User Not Found") {
                return res.status(422).render("userAuth/login", {
                    pageTitle: "Login",
                    path: "/login",
...
exports.postRegister = (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
        return res.status(422).render("userAuth/register", {
            pageTitle: "Register",
            path: "/register",
        ...
        passport.authenticate("local.signup", function (err, user, info) {
            ...
            crypto.randomBytes(32, (err, buffer) => {
                ...
                User.findOne({
                    ...
                    .then((result) => {
                        return res.render("userAuth/confirm-route", {
...
exports.getResetForm = (req, res, next) => {
    return res.render("userAuth/reset", {
...
exports.postResetForm = async (req, res, next) => {
    ...
    if (message) {
        return res.status(422).render("userAuth/reset", {
            pageTitle: "Reset Password",
            path: "/reset",
            errorMessage: message,
        });
    ...
    crypto.randomBytes(32, (err, buffer) => {
        ...
        User.findOne({
            ...
            .then((result) => {
                return res.status(200).render("userAuth/confirm-route", {
                    pageTitle: "Reset password",
                    path: "/confirm-route",
                    role: "reset",
                });
...
exports.getResetPassword = (req, res, next) => {
    ...
    User.findOne({
        ...
        .then((user) => {
            res.render("userAuth/reset-password", {
...
exports.postResetPassword = (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
        return res.status(422).render("userAuth/reset-password", {
            pageTitle: "New password",
...
exports.getConfirmForm = (req, res, next) => {
    ...
    User.findOne({
        ...
        .then((user) => {
            res.render("userAuth/confirm-email", {
                pageTitle: "Verify Email",
                path: "/confirm-email",
                userId: user._id.toString(),
                verifyToken: token,
...
exports.getUserSetting = async (req, res, next) => {
    ...
    const cartFetch = await req.user
        ...
        .then((user) => {
            ...
            res.render("userAuth/user-setting", {
                pageTitle: "Settings",
                path: "/setting",
...
exports.postResetSetting = async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
        return res.status(422).render("userAuth/user-setting", {
            pageTitle: "Settings",
            path: "/setting",
            isAuthenticated: req.isAuthenticated(),
            user: req.user,
